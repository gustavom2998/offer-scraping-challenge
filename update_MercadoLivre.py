...
if __name__=='__main__': 
	...
	try:
		...
		while (novos_links > 0):
			...
			with open(curr_dir + '/Mercado Livre/execution_log.txt','a') as file:
				...
				# Leitura da tabela do Mercado Livre e substituicao de NAs por valores corretos
				link_df = pd.read_csv('Mercado Livre/mercado_livre_offers.csv', sep=";",index_col=0)
				...
				updated_df.to_csv('Mercado Livre/mercado_livre_offers.csv',sep=';')
				
				
				# Configurando o sleep da thread para repetir o processo
				time_sleep = 300 + np.random.randint(0,100)
